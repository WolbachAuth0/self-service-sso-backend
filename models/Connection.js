...
class Connection {
  ...
  static get genericIcon() {
    return "https://raw.githubusercontent.com/WolbachAuth0/image-repo/main/public/connection.png?raw=true"
  }
  static get ticketPattern() {
    return new RegExp(
      `^https://${process.env.VITE_AUTH0_CUSTOM_DOMAIN}/self-service/connections-flow\\?ticket=[a-zA-Z0-9]+$`
    )
  }
  ...
  static async revokeTicket(id) {
    ...
    const response = await management.selfServiceProfiles.revokeSsoTicket({
      id,
      profileId,
    })
    return response.data
  ...
  static async createTicket(connection_config, provisioning_config) {
